...
标注完所有题目后，用 read 重新读取 `raw/{file_id}/output.md`，逐题检查以下内容：

- **答案正确性**：每道题的 `<!-- ANSWER -->` 内容是否与原文答案一致？没有遗漏或错位？
- **格式完整性**：每个 `<!-- QUESTION -->` 块是否包含 `qtype=`、`tags=`、`difficulty=`、`chapter=` 字段？`<!-- ANSWER -->` 和 `<!-- EXPLANATION -->` 分隔是否正确？
- **章节匹配**：`chapter=` 字段是否从上方章节列表中选取的完整名称？
- **内容完整性**：只看 `<!-- QUESTION -->` 块内的内容能否独立解答出这道题？
